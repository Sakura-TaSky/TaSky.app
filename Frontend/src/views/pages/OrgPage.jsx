...
import { CornerDownRight, FolderKanban, LogOut, TriangleAlert } from 'lucide-react';
import { HiUsers } from 'react-icons/hi';
import { BsMicrosoftTeams } from 'react-icons/bs';
import { CgDanger } from 'react-icons/cg';
import { OrgUpdate, setOrg, setOrgErrorMessage, useOrg, useOrgId, useUIState } from '@/global';
...
const OrgPage = () => {
  ...
  return (
    <main className='p-6 '>
      <div className='flex flex-col gap-8'>
        ...
        <div className='flex flex-col gap-3'>
          <div className='flex gap-10'>
            <div className='flex items-center justify-center overflow-hidden w-20 h-20 bg-zinc-500 rounded-full -ml-1'>
              {org?.orgProfilePhoto ? (
                <img className='h-full w-full object-cover' src={org?.orgProfilePhoto} alt={org?.orgName} />
              ) : (
          ...
          <div className='flex flex-col'>
            ...
            <div className='text-sm tracking-wide flex gap-3 items-center'>
              ...
              <img className='h-7 w-7 rounded-full' src={org?.createdBy?.profilePhoto} />
            </div>
          </div>
          <div className='flex flex-col'>
            <span className='text-zinc-500 text-xs'>Description</span>
            <span className='max-w-[350px] break-all text-sm tracking-wide'>{org?.description}</span>
          </div>
          <span className='text-xs text-zinc-500 italic'>Created-At • {org?.createdAt}</span>
        </div>
        {/* memebers info  */}
        <div className='flex flex-col gap-4'>
          <span className='flex items-center gap-2 '>
            <AiOutlineUsergroupAdd size={20} className='text-amber-500' />
            WorkForce
          </span>
          <AvatarGroup
        ...
        <div className='flex flex-col gap-4'>
          ...
          {org?.projects?.length > 0 ? (
            org?.projects?.map(p => (
              <div key={p?._id} className='flex gap-2 text-sm items-center ml-4 text-zinc-600 dark:text-zinc-400'>
        ...
        <div className='flex flex-col gap-4'>
          ...
          {org?.teams?.length > 0 ? (
            org?.teams?.map(t => (
              <div key={t?._id} className='flex gap-2 text-sm items-center ml-4 text-zinc-600 dark:text-zinc-400'>
        ...
        <div className='flex flex-col gap-2'>
          ...
          <p className='max-w-[350px] text-sm ml-5'>
            Leaving this organization will revoke your access to all it's teams, projects, and associated data. You will
            no longer be able to collaborate with it's members unless re-invited. Are you sure you want to proceed ?
          </p>
          <div className='flex ml-6'>
            <CornerDownRight className='text-zinc-500' strokeWidth={1} />
            <IconBtn
              onClick={() => (setShowLeaveOrgPopup(true), dispatch(setOrgErrorMessage('')))}
        ...
        <div className='flex flex-col gap-2'>
          ...
          <div className='max-w-[350px] text-sm ml-5 '>
            ...
            <span className='font-semibold text-red-500'>cannot be undone</span>. This will:
            <ul className='ml-6 mt-1 space-y-0.5'>
              <li> ~ Remove all teams and projects under this organization</li>
              <li> ~ Revoke access for all members</li>
              <li> ~ Delete all pending invites related to this organization</li>
            </ul>
            You must enter the correct organization name to confirm deletion. Only the organization owner can perform
            this action.
          </div>
          <div className='flex ml-6'>
            <CornerDownRight className='text-zinc-500' strokeWidth={1} />
      ...
      {showUpdateOrgPopup && <OrgUpdate setShowUpdateOrgPopup={setShowUpdateOrgPopup} />}
      {showConformation && (
        <Conform
          ...
          cancelText={'Cancle'}
          conformText={'Delete'}
          onCancel={() => (setShowConformation(false), dispatch(setOrgErrorMessage('')))}
      ...
      {showLeaveOrgPopup && (
        <Conform
          ...
          cancelText={'Cancel'}
          conformText={'Leave'}
          onCancel={() => (setShowLeaveOrgPopup(false), dispatch(setOrgErrorMessage('')))}
