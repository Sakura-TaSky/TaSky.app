    ...
    <>
      <div className='relative inter'>
        ...
        >
          ...
          <span className='w-[max-content] absolute group-hover:flex hidden top-9 left-0 text-xs px-2 py-1 bg-zinc-500/20 border border-zinc-500/50 backdrop-blur-2xl rounded-md'>
            Time Line
          </span>
        </div>
        {showTimeLines && (
          <div
            ref={TimelineRef}
            className='hide-scrollbar absolute top-9  w-[310px] sm:w-[350px] h-[84vh] bg-zinc-50 dark:bg-zinc-900 border border-zinc-500/50 rounded-md shadow-md overflow-y-auto z-100'
          >
            ...
            {[...(team?.timeline || [])].reverse().map(item => (
              <div
                key={item._id}
