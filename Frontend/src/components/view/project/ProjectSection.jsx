...
const statusIcon = {
  ...
  'Under Review': (
    <BsEmojiHeartEyesFill size={16} className='text-purple-500' />
  ),
...
const ProjectSection = () => {
  ...
  useClickOutside(taskRef, () => setShowTaskPage(false));
  ...
  if (!section || !section.tasks)
    return (
      <p className='w-full h-50 flex items-center justify-center text-zinc-500 text-sm italic'>
        No Tasks!
      </p>
    );
              ...
              >
                <tr>
                  ...
                  >
                    <ChevronDown
                      size={18}
                      className={`${expandedSections[status] ? '-rotate-180 ' : ''} smooth`}
                    />
                  </th>
                ...
                <tbody>
                  {tasks.length > 0 ? (
                    tasks.map(task => (
                      ...
                      >
                        <td className='border-r p-4 border-gray-500/10'>
                          {statusIcon[status]}
                        </td>
                        <td className='p-4 border-r border-gray-500/10 font-medium'>
                          {task.title}
                          <span className='text-xs text-zinc-500 font-normal line-clamp-1'>
                            {task.description}
                          </span>
                        </td>
                        <td
