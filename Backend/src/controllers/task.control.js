...
const createTask = asyncFunc(async (req, res) => {
  ...
  const isSectionInProject = project.projectSections.find(
  ...
  const memberIds = Array.isArray(assignedToMember)
    ...
    : [assignedToMember];
  if (teamIds.length > 0) {
    const isTeamsInProject = teamIds.every((teamIds) =>
      project.teams.some((t) => t.toString() == teamIds.toString()),
    );
    if (!isTeamsInProject) {
  ...
  const processedSubTasks = (Array.isArray(subTasks) ? subTasks : []).map(
    (st) => {
  ...
  const processedComments = (Array.isArray(comments) ? comments : []).map(
    (c) => {
  ...
  return res
    .status(200)
    .json(new ApiRes(200, populatedTask, "Task created successfully"));
});
...
const updateTask = asyncFunc(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiRes(200, populatedTask, "Task updated successfully"));
});
...
const deleteTask = asyncFunc(async (req, res) => {
  ...
  return res
    .status(201)
    .json(new ApiRes(201, null, "task deleted successfully"));
});
...
const addSubtask = asyncFunc(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiRes(200, popoulatedTask.subTasks, "subTask added successfully"),
    );
});
...
const updateSubtask = asyncFunc(async (req, res) => {
  ...
  const permittedRoles = ["admin", "moderator", "leader"];
  const isPermitted =
    isSubTaskCreaator || isTaskCreator || permittedRoles.includes(userRole);
  if (!isPermitted) {
    throw new ApiError(
      403,
  ...
  return res
    .status(200)
    .json(
      new ApiRes(200, popoulatedTask.subTasks, "subTask updated successfully"),
    );
});
...
const deleteSubtask = asyncFunc(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiRes(200, popoulatedTask.subTasks, "Task removed successfully"),
    );
});
...
const addComment = asyncFunc(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiRes(200, task.comments, "Comment added successfully"));
});
...
const deleteComment = asyncFunc(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiRes(200, populatedTask.comments, "comment deleted successfully"),
    );
});
...
const addLink = asyncFunc(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiRes(200, task.links, "Link added successfully"));
});
...
const deleteLink = asyncFunc(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiRes(200, populatedTask.links, "Link deleted successfully"));
});
...
const addAttachment = asyncFunc(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiRes(200, task.attachments, "Attachment added successfully"));
});
